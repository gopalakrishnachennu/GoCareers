...
class EmployeeDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ).select_related('job', 'job__company_obj', 'consultant').order_by('-created_at')
        ...
        drafts = ResumeDraft.objects.filter(created_by=employee).select_related('job', 'consultant').order_by('-created_at')
        context['resume_drafts'] = drafts[:15]
        context['total_drafts'] = drafts.count()

        cover_letters = CoverLetter.objects.filter(created_by=employee).select_related('job', 'consultant').order_by('-created_at')
