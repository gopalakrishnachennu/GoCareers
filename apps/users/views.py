...
def _get_employee_list_queryset(request):
    ...
    if search_query:
        qs = qs.filter(
            ...
            | Q(employee_profile__designation__name__icontains=search_query)
            | Q(employee_profile__company_name__icontains=search_query)
        )
    return qs.select_related('employee_profile', 'employee_profile__department', 'employee_profile__designation')
...
class EmployeeDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def get_queryset(self):
        return User.objects.filter(role=User.Role.EMPLOYEE).select_related(
            'employee_profile', 'employee_profile__department', 'employee_profile__designation',
        )

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['is_admin'] = True
        context['is_own_profile'] = self.request.user.pk == self.object.pk
        employee = self.object
        ep = getattr(employee, 'employee_profile', None)
        now = timezone.now()

        if not ep:
            return context

        # ── Submissions created by this employee ────────────────────────
        submissions = ApplicationSubmission.objects.filter(
            submitted_by=employee
        ).select_related('job', 'job__company_obj', 'consultant', 'consultant__user').order_by('-created_at')
        context['submissions'] = submissions[:20]
        context['total_submissions'] = submissions.count()

        # Status breakdown
        status_counts = {}
        for s in submissions.values_list('status', flat=True):
        ...
        # ── Resume drafts generated by this employee ────────────────────
        from resumes.models import ResumeDraft, CoverLetter
        drafts = ResumeDraft.objects.filter(created_by=employee).select_related('job', 'consultant', 'consultant__user').order_by('-created_at')
        context['resume_drafts'] = drafts[:15]
        context['total_drafts'] = drafts.count()

        cover_letters = CoverLetter.objects.filter(created_by=employee).select_related('job', 'consultant', 'consultant__user').order_by('-created_at')
        context['cover_letters'] = cover_letters[:10]
        context['total_cover_letters'] = cover_letters.count()

        # ── Consultants this employee manages / has submitted for ───────
        consultant_ids = submissions.values_list('consultant_id', flat=True).distinct()
        from users.models import ConsultantProfile
        managed_consultants = ConsultantProfile.objects.filter(
            pk__in=consultant_ids
        ).select_related('user').order_by('user__first_name')[:20]
        context['managed_consultants'] = managed_consultants

        # ── Designation & Feature Flags ─────────────────────────────────
        context['designation'] = ep.designation
        if ep.designation:
            context['feature_flags'] = ep.designation.allowed_features.all().order_by('sort_order')
        else:
            context['feature_flags'] = []

        # ── Account info ────────────────────────────────────────────────
        context['account_created'] = employee.date_joined
        context['last_login'] = employee.last_login
        context['is_active'] = employee.is_active
