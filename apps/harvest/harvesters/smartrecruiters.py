...
class SmartRecruitersHarvester(BaseHarvester):
    ...
    def _normalize(self, p: dict, slug: str, company_name: str) -> dict:
        ...
        # ── Resolve public-facing URLs ────────────────────────────────────
        # p.get("ref") is the API resource URL (api.smartrecruiters.com/v1/...), NOT the
        # public job page — never use it as original_url.
        # The detail endpoint provides postingUrl/applyUrl which are the real candidate URLs.
        posting_url = ""
        apply_url = ""
        if isinstance(detail, dict) and "error" not in detail:
            posting_url = detail.get("postingUrl") or ""
            apply_url   = detail.get("applyUrl") or posting_url
        if not posting_url:
            posting_url = f"https://jobs.smartrecruiters.com/{api_slug}/{job_id}"
        if not apply_url:
